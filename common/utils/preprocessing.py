...
def augmentation(img, bbox, data_split, input_img_shape, scale_factor=0.25, rot_factor=30, rot_prob=0.6, color_factor=0.2, do_flip=False):
    ...
    # img_color = np.clip(img * color_scale[None, None, :], 0, 255)  # added by Yinqiao Wang

    img, trans, inv_trans = generate_patch_image(img, bbox, scale, rot, do_flip, input_img_shape)

    img = np.clip(img * color_scale[None, None, :], 0, 255)
    # return img, trans, inv_trans, rot, scale, img_color
    return img, trans, inv_trans, rot, scale


def augmentation_with_depth(img, depth_img, hand_mask, bbox, data_split, input_img_shape, scale_factor=0.25, rot_factor=30, rot_prob=0.6, color_factor=0.2, do_flip=False):
    if data_split == "train":
        scale, rot, color_scale = get_aug_config(scale_factor, rot_factor, rot_prob, color_factor)
    else:
        scale, rot, color_scale = 1.0, 0.0, np.array([1, 1, 1])
    ...
    img, depth_img, hand_mask, trans, inv_trans = generate_patch_image_with_depth(img, depth_img, hand_mask, bbox, scale, rot, do_flip, input_img_shape)

    img = np.clip(img * color_scale[None, None, :], 0, 255)
    # return img, trans, inv_trans, rot, scale, img_color
    return img, depth_img, hand_mask, trans, inv_trans, rot, scale
...
def generate_patch_image(cvimg, bbox, scale, rot, do_flip, out_shape):
    ...
    # print('bbox.shape: ', bb_width, bb_height)  # same width and height
    # print('out_shape: ', out_shape) # [128, 128]

    trans = gen_trans_from_patch_cv(bb_c_x, bb_c_y, bb_width, bb_height, out_shape[1], out_shape[0], scale, rot)
    # print('trans: ', trans)   # [2x3], Affine Transformation matrix
    # Do Affine Transformation
    # print('img:', img.shape) # (480, 640, 3)
    # from PIL import Image
    # bf_im = Image.fromarray(np.uint8(img))
    # bf_im.save('before.jpg')

    # img_patch = cv2.warpAffine(img, trans, (int(img_height), int(img_width)), flags=cv2.INTER_LINEAR)
    # af_im = Image.fromarray(np.uint8(img_patch))
    # af_im.save('after_unsize.jpg')

    img_patch = cv2.warpAffine(img, trans, (int(out_shape[1]), int(out_shape[0])), flags=cv2.INTER_LINEAR)
    # af_im = Image.fromarray(np.uint8(img_patch))
    # af_im.save('after.jpg')
    # exit(0)

    # print('img_patch: ', img_patch.shape) # (128, 128, 3)
...
def generate_patch_image_with_depth(cvimg, cv_depth_img, cv_hand_mask, bbox, scale, rot, do_flip, out_shape):
    img = cvimg.copy()
    img_height, img_width, img_channels = img.shape

    depth_img = cv_depth_img.copy()  # (480, 640)
    hand_mask = cv_hand_mask.copy()  # (480, 640)
    ...
    # random_seeds = random.randint(0, 10)
    # from PIL import Image
    # af_im = Image.fromarray(np.uint8(img_patch))
    # af_im.save('rgb_{}.jpg'.format(random_seeds))

    # print('img_patch: ', img_patch.shape) # (128, 128, 3)
    img_patch = img_patch.astype(np.float32)
    inv_trans = gen_trans_from_patch_cv(bb_c_x, bb_c_y, bb_width, bb_height, out_shape[1], out_shape[0], scale, rot, inv=True)

    depth_img_patch = cv2.warpAffine(depth_img, trans, (int(out_shape[1]), int(out_shape[0])), flags=cv2.INTER_LINEAR)  # (256, 256)
    # af_im = Image.fromarray(np.uint8(depth_img_patch))
    # af_im.save('depth_{}.jpg'.format(random_seeds))

    hand_mask_patch = cv2.warpAffine(hand_mask, trans, (int(out_shape[1]), int(out_shape[0])), flags=cv2.INTER_LINEAR)  # (256, 256)

    # print(depth_img_patch.dtype)  # uint16
    depth_img_patch = depth_img_patch.astype(np.float32)

    return img_patch, depth_img_patch, hand_mask_patch, trans, inv_trans
...
def flip_bbox(bbox, image_width):
    ...
    return [image_width - bb_x - 1 - bb_width, bb_y, bb_width, bb_height]
