...
def transform_joint_to_other_db_torch(src_joint, src_name, dst_name):
    src_joint_num = len(src_name)
    dst_joint_num = len(dst_name)

    new_joint = torch.zeros(((src_joint.shape[0], dst_joint_num, ) + src_joint.shape[2:]), dtype=torch.float32).cuda()
    for src_idx in range(len(src_name)):
        name = src_name[src_idx]
        if name in dst_name:
            dst_idx = dst_name.index(name)
