...
class Manager():
    ...
    def write_hparam_to_tb(self, split):
        if not self.accelerator.is_main_process:
            return
        hparam_dict = {
            'exp_name': self.cfg.base.exp_name,
            'model_dir': self.cfg.base.model_dir,
            'input_img_shape': int(self.cfg.data.input_img_shape[0]),
            'model_name': self.cfg.model.name,
            'num_epoch': int(self.cfg.train.num_epochs),
            'batch_size': int(self.cfg.train.batch_size),
            'grad_norm_clip': self.cfg.train.grad_norm_clip,
            'init_lr': self.cfg.optimizer.lr,
            'lr_decay_ms': ','.join(str(int(ms)) for ms in self.cfg.scheduler.milestones),
            'lr_decay_r': self.cfg.scheduler.gamma,
        }
        metric_dict = {k: v.avg for k, v in self.metric_status[split].items()}
        metric_dict['score_best'] = self.score_status[split]['best']
        self.metric_writter.add_hparams(hparam_dict, metric_dict, run_name=self.cfg.base.exp_name)
    ...
    def save_ckpt(self):
        ...
        if self.epoch % self.cfg.summary.save_latest_freq == 0:
            ...
            latest_ckpt_path = osp.join(self.cfg.base.model_dir, "model_e{}.pth".format(self.epoch+1))
