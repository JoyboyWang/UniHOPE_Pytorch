...
class DenseStack(nn.Module):
    ...
    def __init__(self, input_channel, output_channel, ret_mid=False):
...
class DenseStack_Backnone(nn.Module):
    ...
    def __init__(self, input_channel=128, out_channel=24, latent_size=256, kpts_num=21, pretrain=True):
        ...
        self.reduce = conv_layer(out_channel, kpts_num, 1, bn=False, relu=False) # Conv2d(24, 21, kernel_size=(1, 1), stride=(1, 1), bias=False)
    ...
    def forward(self, x):
        pre_out = self.pre_layer(x)  # torch.Size([32, 128, 64, 64]) Conv3*3 + Mobile unit, output_c = input_c
        pre_out_reorg = self.reorg(pre_out)  # Channel shuffle # TODO: understand it
        thrink = self.thrink(pre_out_reorg)  # Conv1*1, output_c = input_c // 4, torch.Size([32, 128, 32, 32])
        ...
        stack2_out, stack2_mid = self.dense_stack2(thrink2) #torch.Size([32, 24, 16, 16]), torch.Size([32, 1024, 4, 4])
        latent = self.mid_proj(stack2_mid) # torch.Size([32, 256, 4, 4])
        uv_reg = self.uv_reg(self.reduce(stack2_out).view(stack2_out.shape[0], 21, -1)) # self.reduce(stack2_out).shape: torch.Size([32, 21, 16, 16]), 21 Joints

        return latent, uv_reg #torch.Size([32, 256, 4, 4]) , torch.Size([32, 21, 2])



class DenseStack_Backnone_2(nn.Module):
    ...
    def __init__(self, input_channel=128, out_channel=24, latent_size=256, kpts_num=21, pretrain=True):
        ...
        Args:
            ...
            pretrain (bool, optional): use pretrain weight or not. Defaults to True.
        """
        super(DenseStack_Backnone_2, self).__init__()
        self.pre_layer = nn.Sequential(conv_layer(3, input_channel // 2, 3, 2, 1), mobile_unit(input_channel // 2, input_channel))
        self.thrink = conv_layer(input_channel * 4, input_channel)
        self.dense_stack1 = DenseStack(input_channel, out_channel)
        self.stack1_remap = conv_layer(out_channel, out_channel)
        ...
        self.mid_proj = conv_layer(1024, latent_size, 1, 1, 0, bias=False, bn=False, relu=False)
        self.reduce = conv_layer(out_channel, kpts_num, 1, bn=False, relu=False) # Conv2d(24, 21, kernel_size=(1, 1), stride=(1, 1), bias=False)
        # directly regress 3d coordinates
        self.xyz_reg = nn.Sequential(linear_layer(latent_size, 128, bn=False), linear_layer(128, 64, bn=False), linear_layer(64, 3, bn=False, relu=False))
        self.reorg = Reorg()
        if pretrain:
            weight = torch.load("model/mob_recon/densestack.pth")
            self.load_state_dict(weight, strict=False)
    ...
    def forward(self, x):
        ...
        stack2_out, stack2_mid = self.dense_stack2(thrink2) #torch.Size([32, 24, 16, 16]), torch.Size([32, 1024, 4, 4])
        latent = self.mid_proj(stack2_mid) # torch.Size([32, 256, 4, 4])
        xyz_reg = self.xyz_reg(self.reduce(stack2_out).view(stack2_out.shape[0], 21, -1)) # self.reduce(stack2_out).shape: torch.Size([32, 21, 16, 16]), 21 Joints

        return latent, xyz_reg #torch.Size([32, 256, 4, 4]) , torch.Size([32, 21, 2])


# a multi-scale version
class DenseStack_Backnone_MS(nn.Module):
    ...
    def __init__(self, input_channel=128, out_channel=24, latent_size=256, kpts_num=21, pretrain=True):
        ...
        Args:
            ...
            pretrain (bool, optional): use pretrain weight or not. Defaults to True.
        """
        super(DenseStack_Backnone_MS, self).__init__()
        self.pre_layer = nn.Sequential(conv_layer(3, input_channel // 2, 3, 2, 1), mobile_unit(input_channel // 2, input_channel))
        self.thrink = conv_layer(input_channel * 4, input_channel)
        self.dense_stack1 = DenseStack(input_channel, out_channel, ret_mid=True)
        self.stack1_remap = conv_layer(out_channel, out_channel)
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.thrink2 = conv_layer((out_channel + input_channel), input_channel)
        self.dense_stack2 = DenseStack2(input_channel, out_channel, final_upsample=False, ret_mid=True)
        self.mid_proj = conv_layer(1024, latent_size, 1, 1, 0, bias=False, bn=False, relu=False)
        self.reduce = conv_layer(out_channel, kpts_num, 1, bn=False, relu=False) # Conv2d(24, 21, kernel_size=(1, 1), stride=(1, 1), bias=False)
        self.uv_reg = nn.Sequential(linear_layer(latent_size, 128, bn=False), linear_layer(128, 64, bn=False), linear_layer(64, 2, bn=False, relu=False))
        self.reorg = Reorg()
    ...
    def forward(self, x):  # torch.Size([B, 3, 128, 128])
        pre_out = self.pre_layer(x)  # torch.Size([B, 128, 64, 64]) Conv3*3 + Mobile unit, output_c = input_c
        pre_out_reorg = self.reorg(pre_out)  # Channel shuffle # torch.Size([B, 512, 32, 32])
        thrink = self.thrink(pre_out_reorg)  # Conv1*1, output_c = input_c // 4, torch.Size([B, 128, 32, 32])
        stack1_out, stack1_mid = self.dense_stack1(thrink)  # stack1_out: torch.Size([B, 24, 32, 32])
        stack1_out_remap = self.stack1_remap(stack1_out)  # Conv1*1, output_c = input_c, torch.Size([B, 24, 32, 32])
        input2 = torch.cat((stack1_out_remap, thrink), dim=1)  # torch.Size([B, 152, 32, 32])
        thrink2 = self.thrink2(input2)  # torch.Size([B, 128, 32, 32])
        stack2_out, stack2_mid = self.dense_stack2(thrink2)  #stack2_out: torch.Size([B, 24, 16, 16]), torch.Size([B, 1024, 4, 4])
        latent = self.mid_proj(stack2_mid[-1]) # torch.Size([B, 256, 4, 4])
        uv_reg = self.uv_reg(self.reduce(stack2_out).view(stack2_out.shape[0], 21, -1)) 

        # import ipdb; ipdb.set_trace()
        # return latent, uv_reg, stack2_mid
        return latent, uv_reg, [pre_out, thrink, stack1_mid[0]]  # [B, 128, 64, 64], [B, 128, 32, 32], [16, 256, 16, 16]
        # return [pre_out, thrink, stack2_out]  # [B, 128, 64, 64], [B, 128, 32, 32], [B, 24, 16, 16]

        # return stack2_mid  # [16, 256, 16, 16], [16, 512, 8, 8], [16, 1024, 4, 4], [16, 1024, 4, 4]
        # return stack1_mid  # [16, 256, 16, 16], [16, 512, 8, 8], [16, 1024, 4, 4]
