...
class mano_regHead(nn.Module):
    ...
    def forward(self, features):
        mano_features = self.mano_base_layer(features)  # torch.Size([16, 512])
        pred_mano_pose_6d = self.pose_reg(mano_features)  # torch.Size([16, 96])

        pred_mano_pose_rotmat = rot6d2mat(pred_mano_pose_6d.view(-1, 6)).view(-1, 16, 3, 3).contiguous()  # torch.Size([16, 16, 3, 3])
        pred_mano_shape = self.shape_reg(mano_features)  # torch.Size([16, 10])
        pred_mano_pose = mat2aa(pred_mano_pose_rotmat.view(-1, 3, 3)).contiguous().view(-1, self.mano_pose_size)  # torch.Size([16, 48])
...
class mano_regShapeHead(nn.Module):

    def __init__(self, mano_layer=mano.layer, feature_size=1024, mano_neurons=[1024, 512]):
        super(mano_regShapeHead, self).__init__()
    ...
    def forward(self, features):
        mano_features = self.mano_base_layer(features)  # torch.Size([B, 512])
        pred_mano_shape = self.shape_reg(mano_features)  # torch.Size([B, 10])

        # features to be distillated
        kd_dict = {'mano_reg_shape': mano_features}

        return {"mano_shape": pred_mano_shape}, kd_dict


class mano_regPoseHead(nn.Module):

    def __init__(self, mano_layer=mano.layer, feature_size=1024, mano_neurons=[1024, 512]):
        super(mano_regPoseHead, self).__init__()
    ...
    def forward(self, features):
        mano_features = self.mano_base_layer(features)  # torch.Size([B, 512])
        pred_mano_pose_6d = self.pose_reg(mano_features)  # torch.Size([B, 96])

        pred_mano_pose_rotmat = rot6d2mat(pred_mano_pose_6d.view(-1, 6)).view(-1, 16, 3, 3).contiguous()  # torch.Size([B, 16, 3, 3])
        pred_mano_pose = mat2aa(pred_mano_pose_rotmat.view(-1, 3, 3)).contiguous().view(-1, self.mano_pose_size)  # torch.Size([B, 48]), including rotation for 15 joints and 1 root joint 

        pred_mano_pose_results = {
            "mano_pose": pred_mano_pose_rotmat,
            "mano_pose_aa": pred_mano_pose
        }

        # features to be distillated
        kd_dict = {'mano_reg_pose': mano_features}

        return pred_mano_pose_results, kd_dict


class mano_regRootPoseHead(nn.Module):

    def __init__(self, mano_layer=mano.layer, feature_size=1024, mano_neurons=[1024, 512, 128, 32]):
        super(mano_regRootPoseHead, self).__init__()

        # 6D representation of rotation matrix
        self.pose6d_size = 1 * 6
        self.mano_pose_size = 1 * 3
    ...
    def forward(self, features):
        mano_features = self.mano_base_layer(features)  # torch.Size([B, 512])
        pred_mano_pose_6d = self.pose_reg(mano_features)  # torch.Size([B, 6])

        # features to be distillated
        kd_dict = {'mano_reg_root_pose': mano_features}

        return pred_mano_pose_6d, kd_dict


class mano_regFingerPoseHead(nn.Module):

    def __init__(self, mano_layer=mano.layer, feature_size=1024, mano_neurons=[1024, 512]):
        super(mano_regFingerPoseHead, self).__init__()

        # 6D representation of rotation matrix
        self.pose6d_size = 15 * 6
        self.mano_pose_size = 15 * 3
    ...
    def forward(self, features):
        mano_features = self.mano_base_layer(features)  # torch.Size([B, 512])
        pred_mano_pose_6d = self.pose_reg(mano_features)  # torch.Size([B, 90])

        # features to be distillated
        kd_dict = {'mano_reg_finger_pose': mano_features}
