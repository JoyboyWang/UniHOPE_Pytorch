...
class hand_regHead(nn.Module):
    ...
    def __init__(self, roi_res=32, joint_nb=21, stacks=1, channels=256, blocks=1):
        ...
        self.betas = nn.Parameter(torch.ones((self.joint_nb, 1), dtype=torch.float32))  # torch.Size([21, 1])
    ...
    def generate_output(self, heatmaps):  # TODO: figure out how it works
    ...
    def forward(self, x):
        out, encoding, preds = [], [], []
        for i in range(self.stacks):
            y = self.hg[i](x)  # torch.Size([16, 256, 32, 32])
            y = self.res[i](y)  # torch.Size([16, 256, 32, 32])
            y = self.fc[i](y)  # torch.Size([16, 256, 32, 32])
            latents = self.score[i](y)  # torch.Size([16, 21, 32, 32])
            heatmaps = self.spatial_softmax(latents)  # torch.Size([16, 21, 32, 32])
            out.append(heatmaps)
            predictions = self.generate_output(heatmaps)  # torch.Size([16, 21, 2])
...
class Residual(nn.Module):
    ...
    def forward(self, x):
        residual = x  # torch.Size([16, 256, 32, 32])
        out = self.bn(x)  
        out = self.leakyrelu(out)  
        out = self.conv1(out)  # torch.Size([16, 128, 32, 32])
        out = self.bn1(out)
        out = self.leakyrelu(out)
        out = self.conv2(out)  # torch.Size([16, 128, 32, 32])
        out = self.bn2(out)
        out = self.leakyrelu(out)
        out = self.conv3(out)  # torch.Size([16, 256, 32, 32])
...
class hand_Encoder(nn.Module):
    ...
    def forward(self, hm_list, encoding_list):
        x = self.heatmap_conv(hm_list[-1]) + self.encoding_conv(encoding_list[-1])  # torch.Size([16, 256, 32, 32])
        ...
        for i in range(self.nRegBlock):  # 4
            ...
            x = self.maxpool(x)  # downsample by 2x

        # x: B x num_feat_chan x 2 x 2
        out = x.view(x.size(0), -1)  # torch.Size([16, 1024])

        return out


class hand_Encoder_2(nn.Module):

    def __init__(self, num_heatmap_chan=21, num_feat_chan=256, size_input_feature=(32, 32), nRegBlock=4, nRegModules=2):
        super(hand_Encoder_2, self).__init__()
