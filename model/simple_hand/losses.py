...
NUM_SHAPES = 10
USE_LEFT = False
# MANO_PARAMS_PATH = "model/simple_hand/"
MANO_PARAMS_PATH = "common/utils/manopth/mano/models"

def load_mano_params_c(use_left=True):
    if use_left:
        # mano_path = os.path.join(MANO_PARAMS_PATH, "MANO_LEFT_C.pkl")
        mano_path = os.path.join(MANO_PARAMS_PATH, "MANO_LEFT.pkl")
    else:
        # mano_path = os.path.join(MANO_PARAMS_PATH, "MANO_RIGHT_C.pkl")
        mano_path = os.path.join(MANO_PARAMS_PATH, "MANO_RIGHT.pkl")
...
def remap_joints_and_fingertip(joints, fingertips):
    ...
    new_joints = new_joints[:, new_idxs]  # NOTE: by yqwang, maybe it is needed
    return new_joints    
...
def mesh_to_joints(mesh: torch.Tensor):
    ...
    J_reg = torch.Tensor(MANO_DATA_RIGHT["J_regressor"].toarray()).to(mesh.device)
