...
from model.kypt_transformer.common.utils.misc import hungarian_match_2djoints, nearest_match_2djoints, batch_rodrigues
from model.kypt_transformer.common.utils.transforms import rot_param_rot_mat
from model.kypt_transformer.common.utils.preprocessing import load_skeleton
...
class CameraParamLoss(nn.Module):
    ...
    def forward(self, joints_gt, joint_valid_in, joint_loc_2d_gt, cam_param_pred, rel_trans_valid, rel_trans_pred):
        ...
        if self.cfg.predict_type == 'vectors' or True:
...
class PoseLoss(nn.Module):
    ...
    def forward(self, pose_out, pose_gt, mano_valid):
        ...
        start_indx = 0
        if self.cfg.hand_type in ['right', 'both']:
            ...
            normalizer = 16
            start_indx += 16
        if self.cfg.hand_type in ['left', 'both']:
...
class ObjPoseLoss(nn.Module):
    ...
    def forward(self, obj_rot_pred, obj_trans_pred, obj_trans_left_pred, hand_rel_trans_pred, obj_rot_gt, obj_trans_gt, hand_rel_trans_gt, obj_bb_rest,
        ...
        if self.cfg.use_obj_rot_parameterization:
...
class ManoMesh():
    ...
    def __init__(self, cfg):
        ...
        # self.mano_layer = {'right': smplx.create(self.cfg.smplx_path, 'mano/models', use_pca=False, is_rhand=True),
        #               'left': smplx.create(self.cfg.smplx_path, 'mano/models', use_pca=False, is_rhand=False)}
        self.mano_layer = {'right': smplx.create(self.cfg.smplx_path, 'mano/models', use_pca=False, is_rhand=True, flat_hand_mean=True),
                      'left': smplx.create(self.cfg.smplx_path, 'mano/models', use_pca=False, is_rhand=False, flat_hand_mean=True)}
        ...
        self.ih26m_joint_regressor = np.load(self.cfg.joint_regr_np_path)
    ...
    def get_mano_mesh(self, pose, shape, rel_trans_in, rel_trans_valid, cam_param):
        ...
        if self.cfg.hand_type in ['right', 'both']:
            mesh_right = self.mano_layer['right'](global_orient=pose[:, 0, :3].to(torch.device('cpu')),
                                                  hand_pose=pose[:, 1:self.cfg.num_joint_queries_per_hand].reshape(pose.shape[0],
                                                                                                              -1).to(
                                                      torch.device('cpu')),
                                                  betas=shape.to(torch.device('cpu'))) # 6N x 778 x 3
            # mesh_right = self.mano_layer['right'](global_orient=pose[:, 0, :3].cuda(),
            #                                       hand_pose=pose[:, 1:self.cfg.num_joint_queries_per_hand].reshape(pose.shape[0],
            #                                                                                                   -1).cuda(),
            #                                       betas=shape.cuda()) # 6N x 778 x 3
            ...
            joints_pred['joints_mano_right'] = joints_pred['joints_mano_right'].view(self.cfg.dec_layers, -1, 21,
                                                                                     3).to(pose.device)  # 6 x N x 21 x 3

            joints_pred['joints_right'] = torch.matmul(self.ih26m_joint_regressor, joints_pred['mesh_right']).view(self.cfg.dec_layers, -1, 21,
                                                                                     3).to(pose.device)  # 6 x N x 21 x 3
            # NOTE: added by yqwang
            joints_pred['joints_right'] = joints_pred['joints_right'][:, :, self.jointsNormalToManoMap]
            
            joints_pred['mesh_right'] = joints_pred['mesh_right'].view(self.cfg.dec_layers, -1,
            ...
            start_indx += self.cfg.num_joint_queries_per_hand
        ...
        if self.cfg.hand_type in ['left', 'both']:
            ...
            # mesh_left = self.mano_layer['left'](global_orient=pose[:, start_indx, :3].cuda(),
            #                                     hand_pose=pose[:, (start_indx + 1):].reshape(pose.shape[0], -1).cuda(),
            #                                     betas=shape.cuda(),
            #                                     transl=rel_trans.cuda()) # 6N x 778 x 3
            ...
            joints_pred['joints_mano_left'] = joints_pred['joints_mano_left'].view(self.cfg.dec_layers, -1, 21,
                                                                                     3).to(pose.device)  # 6 x N x 21 x 3

            joints_pred['joints_left'] = torch.matmul(self.ih26m_joint_regressor, joints_pred['mesh_left']).view(self.cfg.dec_layers, -1, 21,
                                                                                   3).to(pose.device)  # 6 x N x 21 x 3
            # NOTE: added by yqwang
            joints_pred['joints_left'] = joints_pred['joints_left'][:, :, self.jointsNormalToManoMap]
            
            joints_pred['mesh_left'] = joints_pred['mesh_left'].view(self.cfg.dec_layers, -1,
...
class Joints2p5dLoss(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.skeleton = load_skeleton(self.cfg.skeleton_file, 42)
    ...
    def get_1d_gaussian_heatmaps(self, joint_coord):
        ...
        hm_right = torch.exp(-((joint_coord[:, :21, :, None] - x) / self.cfg.sigma) ** 2 / 2)  # N x 21 x 3 x 128
        hm_left = torch.exp(-((joint_coord[:, 21:, :, None] - x) / self.cfg.sigma) ** 2 / 2)  # N x 21 x 3 x 128
...
class JointVectorsLoss(nn.Module):
    ...
    def forward(self, joint_vec_preds_in, joints_gt_in, joint_valid_in, joint_loc_2d_gt, cam_param, rel_trans_pred):
        ...
        for j in range(5):
            for i in range(4):
                if self.cfg.hand_type in ['both', 'right']:
                    if i == 0:
                ...
                if self.cfg.hand_type in ['both', 'left']:
                    offset = 20 if self.cfg.hand_type == 'both' else 0
                    if i == 0:
...
class JointLoss(nn.Module):
    ...
    def forward(self, rel_trans_in, joints_gt_in, joint_valid, joint_loc_2d_gt, rel_trans_valid, joints_pred, cam_param=None):
        ...
        joints_right = joints_pred['joints_right'] - joints_pred['joints_right'][:,:,self.root_joint_idx['right']:self.root_joint_idx['right']+1]
        joints_left = joints_pred['joints_left'] - joints_pred['joints_left'][:,:,self.root_joint_idx['right']:self.root_joint_idx['right']+1]

        joints_gt_right = joints_gt_in[:,:21] - joints_gt_in[:,self.root_joint_idx['right']:self.root_joint_idx['right']+1]
        joints_gt_left = joints_gt_in[:, 21:] - joints_gt_in[:, self.root_joint_idx['left']:self.root_joint_idx['left']+1]
...
class JointClassificationLoss(nn.Module):
    ...
    def forward(self, joint_loc_pred_np, joint_loc_gt, mask, joint_class_pred, joint_valid, peak_joints_map_batch, joint_3d_gt,
                target_obj_kps_heatmap_np, obj_kps_coord_gt_np=None, obj_kps_3d_gt_np=None):
        '''
