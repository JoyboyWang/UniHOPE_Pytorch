...
from model.kypt_transformer.common.utils.transforms import transformManoParamsToCam
import tqdm
...
def augmentation(img, bbox, joint_coord, joint_valid, hand_type, mode, joint_type, mano_pose, mano_trans, mano_shape, joint_cam,
    ...
    if do_flip:
        ...
        mano_trans[:, 0] *= -1
        mano_trans = mano_trans.reshape(-1)
        
        joint_cam[:, 0] *= -1
        
        obj_rot_mat = cv2.Rodrigues(obj_rot)[0]
        obj_rot_mat[0, :] *= -1
        obj_rot = cv2.Rodrigues(obj_rot_mat)[0]
        
        obj_trans[0] *= -1
    
    if mode == 'train':
        trans, scale, rot, color_scale = get_aug_config(cfg)
    ...
    else:  # for validatoin/test set, don't do data augmentation
        trans, scale, rot, color_scale = [0,0], 1.0, 0.0, np.array([1,1,1])
    
    bbox[0] = bbox[0] + bbox[2] * trans[0]
    bbox[1] = bbox[1] + bbox[3] * trans[1]
    img, trans, inv_trans, inv_trans_no_rot, seg_patch, trans_no_rot = generate_patch_image(img, bbox, do_flip, scale, rot, cfg.input_img_shape, obj_seg)
    ...
    # return img, joint_coord, joint_valid, hand_type, inv_trans, inv_trans_no_rot, mano_pose, mano_trans, joint_cam, seg_patch, obj_rot, obj_trans
    return img, joint_coord, joint_valid, hand_type, inv_trans, inv_trans_no_rot, mano_pose, mano_trans, joint_cam, seg_patch, obj_rot, obj_trans, trans_no_rot
# NOTE: there will be a bug when I changed the joint order. use skeleton_ho3d.txt
def transform_input_to_output_space(joint_coord, joint_valid, rel_root_depth, root_valid, root_joint_idx, joint_type, skeleton, cfg):
...
def process_bbox(bbox, original_img_shape, cfg):
...
def generate_patch_image(cvimg, bbox, do_flip, scale, rot, out_shape, obj_seg):
    ...
    return img_patch, trans, inv_trans, inv_trans_no_rot, obj_seg_patch, trans_no_rot
...
class PeakDetector():
    def __init__(self, cfg, nearest_neighbor_th=None):
        self.cfg = cfg
        
        if nearest_neighbor_th is None:
            nearest_neighbor_th = self.cfg.nearest_neighbor_th

        X = np.arange(0, self.cfg.output_hm_shape[0])
        Y = np.arange(0, self.cfg.output_hm_shape[1])
        X, Y = np.meshgrid(X, Y)
        self.pixel_inds = np.reshape(np.stack([Y, X], axis=2), [-1, 2]) # N x 2

        self.dists = []

        filename = 'dists_dict_%d.pickle' % (self.cfg.output_hm_shape[0])
    ...
    def detect_peaks_nms(self, image, num_peaks=200, nearest_neighbor_th=None, intensity_th=None):
        ...
        if intensity_th is None:
            intensity_th = self.cfg.intensity_th
        if nearest_neighbor_th is None:
            nearest_neighbor_th = self.cfg.nearest_neighbor_th
