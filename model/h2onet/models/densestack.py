...
import logging
from model.h2onet.models.modules import conv_layer, mobile_unit, linear_layer, Reorg
...
class H2ONet_Backnone(nn.Module):
    ...
    def __init__(self, cfg, input_channel=128, out_channel=24, latent_size=256, kpts_num=21):
        ...
        self.dense_stack1 = DenseStack(input_channel, out_channel)  # Shared Dual-branch Encoder
        ...
        self.dense_stack2 = DenseStack2(input_channel, out_channel, final_upsample=False)  # Hand Shape Regression
        self.dense_stack3 = DenseStack2_Encoder(input_channel)  # Hand Orientation Regression
    ...
    def forward(self, x):
        ...
        stack1_out = self.dense_stack1(thrink)  # torch.Size([B, 24, 32, 32])
        ...
        stack2_out, stack2_mid = self.dense_stack2(thrink2)  # stack2_out: torch.Size([B, 24, 16, 16]), stack2_mid: torch.Size([B, 1024, 4, 4])
        latent = self.mid_proj(stack2_mid)  # torch.Size([B, 256, 4, 4])
        uv_reg = self.uv_reg(self.reduce(stack2_out).view(stack2_out.shape[0], 21, -1))
        stack3_mid = self.dense_stack3(thrink2.detach())  # torch.Size([B, 1024, 4, 4])

        return latent, stack2_mid, stack3_mid, uv_reg
    


# remove Hand Orientation regression in H2ONet_Backnone
class H2ONet_Backnone_3(nn.Module):
    ...
    def __init__(self, cfg, input_channel=128, out_channel=24, latent_size=256, kpts_num=21):
        ...
        Args:
            ...
            pretrain (bool, optional): use pretrain weight or not. Defaults to True.
        """
        super(H2ONet_Backnone_3, self).__init__()
        self.cfg = cfg
        self.pre_layer = nn.Sequential(conv_layer(3, input_channel // 2, 3, 2, 1), mobile_unit(input_channel // 2, input_channel))
        self.thrink = conv_layer(input_channel * 4, input_channel)
        self.dense_stack1 = DenseStack(input_channel, out_channel)  # Shared Dual-branch Encoder
    ...
    def forward(self, x):
        ...
        return latent, stack2_mid, thrink2, uv_reg


# Not share encoder based on H2ONet_Backnone
class H2ONet_Backnone_4(nn.Module):
    ...
    def __init__(self, cfg, input_channel=128, out_channel=24, latent_size=256, kpts_num=21):
        ...
        Args:
            ...
            pretrain (bool, optional): use pretrain weight or not. Defaults to True.
        """
        super(H2ONet_Backnone_4, self).__init__()
        self.cfg = cfg
        ...
        # Encoder 2
        self.pre_layer_2 = nn.Sequential(conv_layer(3, input_channel // 2, 3, 2, 1), mobile_unit(input_channel // 2, input_channel))
        # self.reorg_2 = Reorg()
        self.thrink_2 = conv_layer(input_channel * 4, input_channel)
        self.dense_stack1_2 = DenseStack(input_channel, out_channel) 
        self.stack1_remap_2 = conv_layer(out_channel, out_channel)
        self.thrink2_2 = conv_layer((out_channel + input_channel), input_channel)
    ...
    def forward(self, x):
        ...
        # 2nd encoder
        pre_out_2 = self.pre_layer_2(x)  # Conv3*3 + Mobile unit, output_c = input_c
        pre_out_reorg_2 = self.reorg(pre_out_2)  # Channel shuffle  # torch.Size([B, 512, 32, 32]
        thrink_2 = self.thrink_2(pre_out_reorg_2)  # Conv1*1, output_c = input_c // 4  # torch.Size([B, 128, 32, 32])
        stack1_out_2 = self.dense_stack1_2(thrink_2)  # torch.Size([B, 24, 32, 32])
        stack1_out_remap_2 = self.stack1_remap_2(stack1_out_2)  # Conv1*1, output_c = input_c
        input2_2 = torch.cat((stack1_out_remap_2, thrink_2), dim=1)  # torch.Size([B, 152, 32, 32])
        thrink2_2 = self.thrink2_2(input2_2)  # torch.Size([B, 128, 32, 32])

        # Hand Orientation Regression
        stack3_mid = self.dense_stack3(thrink2_2)  # torch.Size([B, 1024, 4, 4])
